...
if __name__ == '__main__':
    ...
    FLAGS = {
         ...
         'batch_size': 32,            # 如果爆显存 把batch_size改小一点
        'num_classes': 3,            # 期望分的类别
                 'id': 0,            # 选择不同版本的EfficientNet网络，值为0-7
        'input_size': 224,           # 输入网络的图片分辨率，参数在上面注释中
      'learning_rate': 1e-3,         # 学习率
       'class_weight': False,         # 是否开启class_weight, 当类别不均衡时建议开启
   ...
   'imagenet_weights': 'imagenet',   # 是否加载imagenet预训练权重，默认值为'imagenet'，使用None不加载与训练权重，数据量少时建议开启
    ...
    str2int = {'1':0, '2':1,'3':2}  #将字符标签转换成 int 标签，方便模型训练，训练自己的模型时注意修改为自己的类别
